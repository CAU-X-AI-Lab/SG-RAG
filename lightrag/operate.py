...
async def extract_entities(
    ...
    async def _process_single_content(index: int, chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        # zxzxzxzx打印实体提取信息
        print(f"[DEBUG] [Chunk {index+1} Processing chunk_key: {chunk_key}")
        preview = content[:200].replace('\n', ' ')
        print(f"[DEBUG] [Chunk {index+1} Chunk content preview: {preview}...")
        # zxzxzxzxzx
        ...
        try:
           ...
           if "1301" in error_str and "contentFilter" in error_str:
               logger.warning(
                   f"[BLOCKED] ❌ 内容触发敏感审核，跳过 chunk: {chunk_key}\n"
                   f"Preview: {content[:200].replace(chr(10), ' ')}..."
               )
               with open("blocked_chunks.log", "a", encoding="utf-8") as f:
                   f.write("======== BLOCKED CHUNK ========\n")
                   f.write(f"Chunk Index: {index}\n")
                   f.write(f"Chunk Key: {chunk_key}\n")
                   f.write(f"Chunk Preview: {content[:200].replace(chr(10), ' ')}...\n")
                   f.write(f"Full Chunk Text:\n{content}\n\n")
           else:
               logger.warning(
                   f"[SKIPPED] ⚠️ 非内容审核异常跳过 chunk: {chunk_key}\n"
                   f"Exception: {error_str}\n"
                   f"Preview: {content[:200].replace(chr(10), ' ')}..."
               )
           return {}, {}  # 无论是哪种异常都跳过该 chunk
    ...
    for result in tqdm_async(
        asyncio.as_completed([_process_single_content(i, c) for i, c in enumerate(ordered_chunks)]),
        ...
        try:
            ...
            logger.warning(f"[FATAL] ❌ 异步任务失败，跳过该chunk任务\n{str(e)}")
            import traceback
            traceback.print_exc()
            continue
        # zxzxzxzxzxzxzx
